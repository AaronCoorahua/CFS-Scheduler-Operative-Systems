...
uint64_t RBT_RED = 0;
uint64_t RBT_BLACK = 1;

// Puntero a la raíz del árbol de nodos RBT.
uint64_t *rbt_root;

// Puntero al nodo sentinela que representa las hojas nulas del árbol.
uint64_t *RBT_NIL;

// Puntero a la cabeza de la lista de nodos RBT reciclables.
uint64_t *free_rbt_nodes;

// rbt_node
// +----+-----------+---------------------------------+
// |  0 | color     | RBT_RED o RBT_BLACK             |
// |  1 | parent    | puntero al nodo padre en el RBT |
// |  2 | left      | puntero al hijo izquierdo       |
// |  3 | right     | puntero al hijo derecho         |
// |  4 | context   | puntero al machine_context real |
// +----+-----------+---------------------------------+

uint64_t RBT_NODE_ENTRIES = 5;

uint64_t get_rbt_node_color(uint64_t *node) { return *node; }
uint64_t *get_rbt_node_parent(uint64_t *node) { return (uint64_t *)*(node + 1); }
uint64_t *get_rbt_node_left(uint64_t *node) { return (uint64_t *)*(node + 2); }
uint64_t *get_rbt_node_right(uint64_t *node) { return (uint64_t *)*(node + 3); }
uint64_t *get_rbt_node_context(uint64_t *node) { return (uint64_t *)*(node + 4); }

void set_rbt_node_color(uint64_t *node, uint64_t color) { *node = color; }
void set_rbt_node_parent(uint64_t *node, uint64_t *parent) { *(node + 1) = (uint64_t)parent; }
void set_rbt_node_left(uint64_t *node, uint64_t *left) { *(node + 2) = (uint64_t)left; }
void set_rbt_node_right(uint64_t *node, uint64_t *right) { *(node + 3) = (uint64_t)right; }
void set_rbt_node_context(uint64_t *node, uint64_t *context) { *(node + 4) = (uint64_t)context; }

uint64_t *allocate_rbt_node() {
  uint64_t *new_node;
  if (free_rbt_nodes == (uint64_t *)0) {
    new_node = smalloc(RBT_NODE_ENTRIES * sizeof(uint64_t));
  } else {
    new_node = free_rbt_nodes;
    free_rbt_nodes = get_rbt_node_right(new_node);
  }
  return new_node;
}

void free_rbt_node(uint64_t *node_to_free) {
  if (node_to_free == (uint64_t *)0) {
    return;
  }
  set_rbt_node_right(node_to_free, free_rbt_nodes);
  free_rbt_nodes = node_to_free;
}
...
void rbt_insert_fixup(uint64_t *z_node);
void rbt_delete_fixup(uint64_t *x_node);
void rbt_transplant(uint64_t *u_node, uint64_t *v_node);
...
void reset_microkernel()
{
  ...
  while (used_contexts != (uint64_t *)0)
    delete_context_and_rbt_node(used_contexts);
...
void implement_fork(uint64_t *context){
  ...
  // RED BLACK TREE
  child_c = create_context_and_rbt_node(MY_CONTEXT, 0);
...
// Inicializa las variables globales del RBT (rbt_root, RBT_NIL, free_rbt_nodes)
void rbt_init() {
    // Alocar memoria para el nodo sentinela RBT_NIL.
    RBT_NIL = allocate_rbt_node();

    // Configurar el nodo RBT_NIL.
    // - Su color es siempre NEGRO.
    // - Su padre, hijo izquierdo e hijo derecho apuntan a sí mismo.
    // - No apunta a ningún machine_context real.
    set_rbt_node_color(RBT_NIL, RBT_BLACK);
    set_rbt_node_parent(RBT_NIL, RBT_NIL);
    set_rbt_node_left(RBT_NIL, RBT_NIL);
    set_rbt_node_right(RBT_NIL, RBT_NIL);
    set_rbt_node_context(RBT_NIL, (uint64_t*)0);

    // Inicializar la raíz del árbol para que apunte a RBT_NIL.
    // Un árbol vacío tiene su raíz apuntando al sentinela.
    rbt_root = RBT_NIL;

    // Inicializar la lista de nodos reciclabes como vacía.
...
void rbt_insert(uint64_t *node_to_insert) {
    ...
    set_rbt_node_parent(node_to_insert, y_node);
    ...
    // Configuramos el nuevo nodo.
    set_rbt_node_left(node_to_insert, RBT_NIL);
    set_rbt_node_right(node_to_insert, RBT_NIL);
    set_rbt_node_color(node_to_insert, RBT_RED);
    
    // Llamamos a la función de rebalanceo para mantener las propiedades del RBT.
    rbt_insert_fixup(node_to_insert);
}
...
void rbt_delete(uint64_t *node_to_delete) {
    ...
    if (get_rbt_node_left(node_to_delete) == RBT_NIL) {
        ...
        rbt_transplant(node_to_delete, get_rbt_node_right(node_to_delete));
    } else if (get_rbt_node_right(node_to_delete) == RBT_NIL) {
        x_node = get_rbt_node_left(node_to_delete);
        rbt_transplant(node_to_delete, get_rbt_node_left(node_to_delete));
    } else {
        // Encontrar el sucesor in-order (el más pequeño del subárbol derecho)
        y_node = get_rbt_node_right(node_to_delete);
        while (get_rbt_node_left(y_node) != RBT_NIL) {
            y_node = get_rbt_node_left(y_node);
        }

        y_original_color = get_rbt_node_color(y_node);
        x_node = get_rbt_node_right(y_node);
        if (get_rbt_node_parent(y_node) == node_to_delete) {
            set_rbt_node_parent(x_node, y_node);
        } else {
            rbt_transplant(y_node, get_rbt_node_right(y_node));
            set_rbt_node_right(y_node, get_rbt_node_right(node_to_delete));
            set_rbt_node_parent(get_rbt_node_right(y_node), y_node);
        }
        rbt_transplant(node_to_delete, y_node);
        set_rbt_node_left(y_node, get_rbt_node_left(node_to_delete));
        set_rbt_node_parent(get_rbt_node_left(y_node), y_node);
        set_rbt_node_color(y_node, get_rbt_node_color(node_to_delete));
    }
    if (y_original_color == RBT_BLACK) {
        rbt_delete_fixup(x_node);
    }
}
...
uint64_t *rbt_find_minimum() {
    ...
    while (get_rbt_node_left(current_node) != RBT_NIL) {
        current_node = get_rbt_node_left(current_node);
    }
    // Cuando ya no podemos ir más a la izquierda, hemos encontrado el mínimo.
    return current_node;
}

//rbt_search_by_id: Busca un nodo en el árbol que corresponda a un ID específico.
...
uint64_t *rbt_search_by_id(uint64_t id_to_find) {
    ...
    while (current_node != RBT_NIL) {
        ...
        if (id_to_find == current_id) {
            ...
            return current_node;
        } else if (id_to_find < current_id) {
            // El ID que buscamos es más pequeño, vamos por la izquierda.
            current_node = get_rbt_node_left(current_node);
        } else {
            // El ID que buscamos es más grande, vamos por la derecha.
            current_node = get_rbt_node_right(current_node);
        }
    }
  

    // Si salimos del bucle, significa que llegamos a una hoja NIL
    // sin encontrar el nodo. Devolvemos RBT_NIL para indicar que no se encontró.
    return RBT_NIL;
}
...
void print_rbt() {
    ...
    printf("\n---------- INICIO ÁRBOL DE CONTEXTOS ----------\n");

    current_node = rbt_root;
    depth = 0;

    while (1) {
        if (current_node != RBT_NIL) {
            if (stack_top >= MAX_TREE_HEIGHT) {
                printf("Error: Árbol demasiado profundo para imprimir.\n");
                return;
            }
          
            node_stack[stack_top] = current_node;
            ...
            if (stack_top > 0) {
                stack_top = stack_top - 1;
                current_node = node_stack[stack_top];
...
void left_rotate(uint64_t *x_node) {
    ...
    if (get_rbt_node_left(y_node) != RBT_NIL)
        set_rbt_node_parent(get_rbt_node_left(y_node), x_node);

    set_rbt_node_parent(y_node, get_rbt_node_parent(x_node));

    if (get_rbt_node_parent(x_node) == RBT_NIL)
        rbt_root = y_node;
    else if (x_node == get_rbt_node_left(get_rbt_node_parent(x_node)))
        set_rbt_node_left(get_rbt_node_parent(x_node), y_node);
    else
        set_rbt_node_right(get_rbt_node_parent(x_node), y_node);
    
    set_rbt_node_left(y_node, x_node);
    set_rbt_node_parent(x_node, y_node);
}
...
void right_rotate(uint64_t *y_node) {
    ...
    if (get_rbt_node_right(x_node) != RBT_NIL)
        set_rbt_node_parent(get_rbt_node_right(x_node), y_node);

    set_rbt_node_parent(x_node, get_rbt_node_parent(y_node));

    if (get_rbt_node_parent(y_node) == RBT_NIL)
        rbt_root = x_node;
    else if (y_node == get_rbt_node_right(get_rbt_node_parent(y_node)))
        set_rbt_node_right(get_rbt_node_parent(y_node), x_node);
    else
        set_rbt_node_left(get_rbt_node_parent(y_node), x_node);
        
    set_rbt_node_right(x_node, y_node);
    set_rbt_node_parent(y_node, x_node);
}

//rbt_insert_fixup: Restaura las propiedades del RBT después de una inserción.
void rbt_insert_fixup(uint64_t *z_node) {
    uint64_t *y_node;

    while (get_rbt_node_color(get_rbt_node_parent(z_node)) == RBT_RED) {
        if (get_rbt_node_parent(z_node) == get_rbt_node_left(get_rbt_node_parent(get_rbt_node_parent(z_node)))) {
            y_node = get_rbt_node_right(get_rbt_node_parent(get_rbt_node_parent(z_node)));
            if (get_rbt_node_color(y_node) == RBT_RED) {
                set_rbt_node_color(get_rbt_node_parent(z_node), RBT_BLACK);
                set_rbt_node_color(y_node, RBT_BLACK);
                set_rbt_node_color(get_rbt_node_parent(get_rbt_node_parent(z_node)), RBT_RED);
                z_node = get_rbt_node_parent(get_rbt_node_parent(z_node));
            } else {
                if (z_node == get_rbt_node_right(get_rbt_node_parent(z_node))) {
                    z_node = get_rbt_node_parent(z_node);
                    left_rotate(z_node);
                }
                set_rbt_node_color(get_rbt_node_parent(z_node), RBT_BLACK);
                set_rbt_node_color(get_rbt_node_parent(get_rbt_node_parent(z_node)), RBT_RED);
                right_rotate(get_rbt_node_parent(get_rbt_node_parent(z_node)));
            }
        } else {
            y_node = get_rbt_node_left(get_rbt_node_parent(get_rbt_node_parent(z_node)));
            if (get_rbt_node_color(y_node) == RBT_RED) {
                set_rbt_node_color(get_rbt_node_parent(z_node), RBT_BLACK);
                set_rbt_node_color(y_node, RBT_BLACK);
                set_rbt_node_color(get_rbt_node_parent(get_rbt_node_parent(z_node)), RBT_RED);
                ...
                if (z_node == get_rbt_node_left(get_rbt_node_parent(z_node))) {
                    z_node = get_rbt_node_parent(z_node);
                    right_rotate(z_node);
                }
                set_rbt_node_color(get_rbt_node_parent(z_node), RBT_BLACK);
                set_rbt_node_color(get_rbt_node_parent(get_rbt_node_parent(z_node)), RBT_RED);
                left_rotate(get_rbt_node_parent(get_rbt_node_parent(z_node)));
            }
        }
    }
    set_rbt_node_color(rbt_root, RBT_BLACK);
}

//rbt_transplant: Reemplaza un subárbol en la posición 'u' con el subárbol en 'v'.
void rbt_transplant(uint64_t *u_node, uint64_t *v_node) {
    if (get_rbt_node_parent(u_node) == RBT_NIL) {
        rbt_root = v_node;
    } else if (u_node == get_rbt_node_left(get_rbt_node_parent(u_node))) {
        set_rbt_node_left(get_rbt_node_parent(u_node), v_node);
    } else {
        set_rbt_node_right(get_rbt_node_parent(u_node), v_node);
    }
...
uint64_t selfie_run(uint64_t machine)
{
  ...
  // RED BLACK TREE
  current_context = create_context_and_rbt_node(MY_CONTEXT, 0);
